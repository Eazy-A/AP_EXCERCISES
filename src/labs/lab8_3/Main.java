...
class GameRoom {
    ...
    private void startProcessor() {
        executor.execute(() -> {
                while (!actionQueue.isEmpty() || running) {
                    PlayerAction playerAction;
                    try {
                        playerAction = actionQueue.poll(100, TimeUnit.MILLISECONDS);
...
class GameServer {
        ...
        Thread dispatcherThread = new Thread(() -> {
            while (!inputQueue.isEmpty() || running){
                try {
                    ...
                    GameRoom gameRoom = rooms.computeIfAbsent(roomId, GameRoom::new);
                    gameRoom.submitAction(roomAction.action);
    ...
    public void shutdown() {
        ...
        System.out.println("Shutting down all rooms...");
