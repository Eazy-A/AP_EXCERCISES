...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor <T> firstLogProcessor = (logsList) -> logsList.stream()
                .filter(logs -> logs.getType().equals("INFO"))
                .collect(Collectors.toCollection(ArrayList::new));

                LogProcessor <T> secondLogProcessor = (logList) -> logList.stream()
                        .filter(logs -> logs.getMessage().length() < 100)
                        .collect(Collectors.toCollection(ArrayList::new));

                        LogProcessor <T> thirdLogProcessor = (logList) -> logList.stream()
